...
import { fallbackToVercelWhenNoNotion } from "@/lib/notion-fallback"
...
// Helper function to fetch and process blog posts from Notion
async function fetchBlogPostsFromNotion(queryBody: Record<string, unknown> = {}) {
  ...
  const blogPosts = await Promise.all(
    data.results.map(async (page: { id: string; properties: Record<string, unknown>; cover?: { type: string; file?: { url: string }; external?: { url: string } } | null; icon?: { type?: string; emoji?: string; file?: { url: string }; external?: { url: string } } | null; last_edited_time: string; url: string; created_time: string }, index: number) => {
      console.log(`Processing blog post ${index + 1}/${data.results.length}: ${page.id}`)
      ...
      console.log("Available blog post properties:", Object.keys(properties))
      ...
      console.log(`📝 Final blog post "${title}" - Image: ${image}, Featured: ${featured}`)
  ...
  // Log each blog post for debugging
  blogPosts.forEach((post, index) => {
    console.log(
      `Blog Post ${index + 1}: "${post.title}" - Featured: ${post.featured}, Category: ${post.category}`,
    )
  })
...
export async function GET(request: NextRequest) {
  const fallback = await fallbackToVercelWhenNoNotion(request, "/api/blog/")
