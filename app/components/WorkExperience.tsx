...
import type { YearGroup, Position, WorkExperienceItem } from "../types/types"
import { queryNotionDatabase, WORK_EXPERIENCE_DATABASE_ID, transformWorkExperience } from "@/lib/notion-morphic"
...
export default function WorkExperience() {
  ...
  useEffect(() => {
    async function fetchWorkExperience() {
      try {
        const data = await queryNotionDatabase(WORK_EXPERIENCE_DATABASE_ID)
        const workItems = data.results.map(transformWorkExperience)

        // Group work experience by year
        const groupedByYear = workItems.reduce((acc: Record<string, Position[]>, job: WorkExperienceItem) => {
